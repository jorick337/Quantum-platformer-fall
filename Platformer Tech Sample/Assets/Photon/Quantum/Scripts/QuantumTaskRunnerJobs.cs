...
public class QuantumTaskRunnerJobs : MonoBehaviour, IDisposable, Photon.Deterministic.IDeterministicPlatformTaskRunner {
...
#else
  protected int DefaultQuantumJobsMaxCount => JobsUtility.MaxJobThreadCount;
#endif
  ...
  private void Schedule(Action[] delegates, int jobsMaxCount) {
    ...
    if (_jobs == null || _jobs.Length != jobsCount) {
      ...
      _handles = new NativeArray<JobHandle>(jobsCount, Allocator.Persistent);
    }
    Profiler.EndSample();
