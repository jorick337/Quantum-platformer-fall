  ...
  public static class MapNavMeshBaker {
    ...
    public static NavMesh BakeNavMesh(Map map, MapNavMesh.BakeData navmeshBakeData) {
      ...
      using (var progressBar = new ProgressBar("Baking Quantum NavMesh", true, false)) {
        ...
        if (navmeshBakeData.Links != null) {
          ...
          GenerateNavMeshLinks(navmesh, navmeshBakeData.Links, map.Regions, navmeshBakeData.LinkErrorCorrection, navmeshBakeData.EnableQuantum_XY, p => progressBar.Progress = p);
        }
    ...
    static NavMeshTriangleNode[] GenerateTriangleGrid(NavMesh navmesh, Action<float> reporter) {
      ...
      for (int i = 0; i < navmesh.Triangles.Length; ++i) {
        ...
        int yMin = 0;
        int yMax = 0;
        if (!LocateGridCells(ref xMin, ref xMax, ref yMin, ref yMax, navmesh.WorldOffset, navmesh.GridNodeSize, navmesh.GridSizeX, navmesh.GridSizeY, v0.XZ, v1.XZ, v2.XZ)) {
          continue;
        }
    ...
    static bool TriangleContains(NavMeshTriangle[] ts, NavMeshVertex[] vs, FPVector3 point, NavMeshRegionMask regions) {
      for (int i = 0; i < ts.Length; ++i) {
        ...
        // Check if one vertex of the checked triangle matches the height of the border
        bool heightMatches = FPMath.Abs(point.Y - vs[ts[i].Vertex0].Point.Y) < FP.Epsilon;
        heightMatches |= FPMath.Abs(point.Y - vs[ts[i].Vertex1].Point.Y) < FP.Epsilon;
        heightMatches |= FPMath.Abs(point.Y - vs[ts[i].Vertex2].Point.Y) < FP.Epsilon;
        if (heightMatches == false) {
          continue;
        }
    ...
    static NavMeshBorderNode[] GenerateBorderGrid(NavMesh navmesh, Action<float> reporter) {
      ...
      for (int b = 0; b < navmesh.Borders.Length; ++b) {
        ...
        int yMin = 0;
        int yMax = 0;
        if (!LocateGridCells(ref xMin, ref xMax, ref yMin, ref yMax, navmesh.WorldOffset, navmesh.GridNodeSize, navmesh.GridSizeX, navmesh.GridSizeY, p0.XZ, p1.XZ)) {
          continue;
        }
    ...
    public static bool LocateGridCells(ref int xMin, ref int xMax, ref int yMin, ref int yMax, FPVector2 worldOffset, int gridNodeSize, int gridSizeX, int gridSizeY, params FPVector2[] positions) {
      ...
      xMax = Math.Min(xMax, gridSizeX - 1);
      yMax = Math.Min(yMax, gridSizeY - 1);
    ...
    static void GenerateNavMeshLinks(NavMesh navmesh, MapNavMeshLink[] links, string[] regions, bool errorCorrection, bool enableQuantumXY, Action<float> reporter) {
      ...
      for (var l = 0; l < links.Length; l++) {
        ...
        var link = links[l];
        var linkStart = link.Start;
        var linkEnd = link.End;
#if QUANTUM_XY
        if (enableQuantumXY) {
          linkStart = new Vector3(linkStart.x, linkStart.z, linkStart.y);
          linkEnd = new Vector3(linkEnd.x, linkEnd.z, linkEnd.y);
        }
#endif
        var startTriangle = -1;
        var startPosition = linkStart.ToFPVector3();
        var endTriangle = -1;
        var endPosition = linkEnd.ToFPVector3();

        checkedTriangles.Clear();

        {
          // find cell index (expand one cell for error correction)
          var p = startPosition.XZ - navmesh.WorldOffset;
          ...
          for (int x = xMin; x <= xMax; x++) {
            for (int z = zMin; z <= zMax; z++) {
              ...
              for (int t = 0; t < navmesh.TrianglesGrid[i].Triangles.Length; t++) {
                ...
                var d = Vector3Double.ClosestDistanceToTriangle(new Vector3Double(linkStart),
        ...
        {
          // find cell index (expand one cell for error correction)
          var p = endPosition.XZ - navmesh.WorldOffset;
          var _x = (p.X / navmesh.GridNodeSize).AsInt;
          var _z = (p.Y / navmesh.GridNodeSize).AsInt;
          var closestDistance = double.MaxValue;
          ...
          for (int x = xMin; x <= xMax; x++) {
            for (int z = zMin; z <= zMax; z++) {
              ...
              for (int t = 0; t < navmesh.TrianglesGrid[i].Triangles.Length; t++) {
                var triangleIndex = navmesh.TrianglesGrid[i].Triangles[t];
                var closestPoint = new Vector3Double();
                var d = Vector3Double.ClosestDistanceToTriangle(new Vector3Double(linkEnd),
                                                                new Vector3Double(navmesh.Vertices[navmesh.Triangles[triangleIndex].Vertex0].Point),
                                                                new Vector3Double(navmesh.Vertices[navmesh.Triangles[triangleIndex].Vertex1].Point),
                                                                new Vector3Double(navmesh.Vertices[navmesh.Triangles[triangleIndex].Vertex2].Point),
                                                                ref closestPoint);
