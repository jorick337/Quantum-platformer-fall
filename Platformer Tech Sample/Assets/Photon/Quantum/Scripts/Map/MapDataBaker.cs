...
#pragma warning restore CS0618 // Type or member is obsolete
  ...
  public static void BakeColliders(MapData data, Boolean inEditor) {
    ...
    foreach (var c in FindLocalObjects<QuantumStaticPolygonCollider2D>(scene)) {
      ...
      if (c.BakeAsStaticEdges2D) {
        for (var i = 0; i < c.Vertices.Length; i++) {
          if (BakeStaticEdge2D(c.transform, c.PositionOffset, c.RotationOffset, c.Vertices[i], c.Vertices[(i + 1) % c.Vertices.Length], c.Height, c.Settings, staticCollider2DList.Count, out var staticEdge)) {
            staticCollider2DList.Add(staticEdge);
            data.StaticCollider2DReferences.Add(c);
          } else {
            Debug.LogWarning($"Degenerate edge on {nameof(QuantumStaticPolygonCollider2D)} '{c.name}', between vertices '{i}' ({c.Vertices[i]}) and '{(i + 1) % c.Vertices.Length}' ({c.Vertices[(i + 1) % c.Vertices.Length]}). The edge will not be baked.");
          }
        }
    ...
    foreach (var c in FindLocalObjects<QuantumStaticEdgeCollider2D>(scene)) {
      ...
      if (BakeStaticEdge2D(c.transform, c.PositionOffset, c.RotationOffset, c.VertexA, c.VertexB, c.Height, c.Settings, staticCollider2DList.Count, out var staticEdge)) {
        staticCollider2DList.Add(staticEdge);
        data.StaticCollider2DReferences.Add(c);
      } else {
        Debug.LogWarning($"Degenerate edge on {nameof(QuantumStaticEdgeCollider2D)} '{c.name}', between vertices A ({c.VertexA}) and B ({c.VertexB}). The edge will not be baked.");
      }
    }
    ...
    data.Asset.Settings.StaticColliders3DTriangles = new SortedDictionary<int, TriangleCCW[]>();
...
#if (!UNITY_2019_1_OR_NEWER || QUANTUM_ENABLE_PHYSICS2D) && !QUANTUM_DISABLE_PHYSICS2D
  static bool BakeStaticEdge2D(Transform t, FPVector2 positionOffset, FP rotationOffset, FPVector2 vertexA, FPVector2 vertexB, FP height, QuantumStaticColliderSettings settings, int colliderId, out MapStaticCollider2D bakedCollider) {
    ...
    var edgeExtent = (startToEnd.magnitude / 2.0f).ToFP();
    
    bakedCollider = new MapStaticCollider2D {
