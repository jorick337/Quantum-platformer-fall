    ...
    public virtual void OnGUI() {
      ...
      if (QuantumCoreDllInfo != null) {
        try {
          ...
          if (string.IsNullOrEmpty(QuantumCoreDllInfo.Comments) == false) {
            EditorGUILayout.TextField("Custom", QuantumCoreDllInfo.Comments);
          }
        } catch {
...
namespace Quantum.Editor {
  ...
  static class QuantumObjectFactory {
    ...
    private static void CreateCharacterControllerEntity(MenuCommand mc) =>
      ...
      .ThenAdd<EntityComponentCharacterController3D>()
      .ThenAlter<EntityComponentCharacterController3D>(x => x.Prototype.Config.Id = CharacterController3DConfig.DEFAULT_ID)
      .ThenAdd<global::EntityView>()
