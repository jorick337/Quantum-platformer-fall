    ...
    void OnInspectorGUI(DeterministicSessionConfigAsset asset) {
      using (new QuantumEditorGUI.BoxScope("Deterministic Session Config")) {
        ...
        using (new QuantumEditorGUI.SectionScope("Simulation")) {
          ...
          asset.Config.LockstepSimulation = EditorGUILayout.Toggle("Force Strict Lockstep", asset.Config.LockstepSimulation);
          HelpBox("Legacy feature that forces the Quantum simulation to run in strict lockstep mode, where no rollbacks are performed.\n" +
                  "Most games suitable for lockstep are better off using a soft lockstep instead by introducing enough input offset (recommended at least 10 ticks), " +
                  "while still being able to perform predictions if and when necessary.");
        ...
        using (new QuantumEditorGUI.SectionScope("Input")) {
          ...
          HelpBox("The minimum input offset a player can have. Defined in ticks.");
          ...
          HelpBox("The maximum input offset a player can have. Defined in ticks.");

          asset.Config.InputDelayPingStart = Math.Max(0, EditorGUILayout.IntField("Offset Ping Start", asset.Config.InputDelayPingStart));
          HelpBox("At what ping value that Quantum starts applying input offset. Defined in milliseconds.");
...
namespace Quantum.Editor {
  ...
  public class EntityPrototypeEditor : UnityEditor.Editor {
    ...
    [NonSerialized]
    private List<Type> _commonComponentTypes = null;

    [NonSerialized]
    private List<SerializedObject> _componentSerializedObjects = null;
    ...
    public override void OnInspectorGUI() {
      ...
      if (QuantumEditorSettings.Instance.EntityComponentInspectorMode != QuantumEntityComponentInspectorMode.ShowMonoBehaviours) {
        ...
        using (new EditorGUILayout.VerticalScope(GUI.skin.box)) {
          ...
          if (_commonComponentTypes == null) {
            var groups = targets.Cast<EntityPrototype>()
             .SelectMany(x => x.GetComponents<EntityComponentBase>())
             .Where(x => x)
             .GroupBy(x => x.GetType());
            
            _commonComponentTypes       = new List<Type>();
            _componentSerializedObjects = new List<SerializedObject>();

            foreach (var group in groups) {
              if (group.Count() == targets.Length) {
                _commonComponentTypes.Add(group.Key);
                _componentSerializedObjects.Add(new SerializedObject(group.Cast<UnityEngine.Object>().ToArray()));
              }
            }
          }
          ...
          { 
            ...
            if (GUI.Button(buttonRect, "+", EditorStyles.miniButton)) {
              ...
              var availableComponents = componentPrototypeTypes
                .Where(x => !_commonComponentTypes.Contains(x))
                .ToList();
          ...
          using (new EditorGUI.IndentLevelScope()) {
            foreach (var so in _componentSerializedObjects) {
              
              var sp = so.GetIterator();
              var c  = ((EntityComponentBase)so.targetObject);

              var rect = GUILayoutUtility.GetRect(GUIContent.none, skin.inspectorTitlebar);
              sp.isExpanded = EditorGUI.InspectorTitlebar(rect, sp.isExpanded, so.targetObjects, true);
  ...
  public class MapAssetEditor : AssetBaseEditor {
    ...
    public override void OnInspectorGUI() {
      ...
      foreach (MapAsset data in targets) {
        ...
        data.Settings.EnsurePhysicsWorldSizeInValidRange();
...
namespace Quantum.Editor {
#if (!UNITY_2019_1_OR_NEWER || QUANTUM_ENABLE_AI) && !QUANTUM_DISABLE_AI
  using UnityEngine;
  using UnityEditor;
  using System.Linq;
  using System.Collections.Generic;
    ...
    public static bool BakeUnityNavmesh(GameObject go) {
      if (MapNavMesh.NavMeshSurfaceType == null) {
        ...
        try {
          ...
          var type = typeof(ScriptableSingleton<>).MakeGenericType(MapNavMesh.NavMeshAssetManagerType);
          var instance = type.GetProperty("instance", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public).GetValue(null, null);
          var surfaceObjects = surfaces.Select(s => s.GetComponent(MapNavMesh.NavMeshSurfaceType)).ToArray();
          foreach (var s in surfaceObjects) {
            var assetToDelete = (Object)MapNavMesh.NavMeshAssetManagerType.GetMethod("GetNavMeshAssetToDelete", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).Invoke(instance, new object[] { s });
            if (string.IsNullOrEmpty(AssetDatabase.GetAssetPath(assetToDelete)) == false) {
              ...
              MapNavMesh.NavMeshAssetManagerType.GetMethod("ClearSurface", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).Invoke(instance, new object[] { s });
            } else {
              // Only clear the surface data
              MapNavMesh.NavMeshSurfaceType.GetMethod("RemoveData").Invoke(s, null);
            }
          }
        } catch (System.Exception e) {
          Debug.LogWarning($"Failed to reset Unity navmesh surfaces due to an exception: {e.Message}");
        }
    ...
    public partial class MapNavMeshUnityEditor : UnityEditor.Editor {
      public override void OnInspectorGUI() {
        ...
        using (new QuantumEditorGUI.BoxScope("Import Settings")) {
          ...
          if (data.FixTrianglesOnEdges) {
            data.FixTrianglesOnEdgesEpsilon = Mathf.Max(data.FixTrianglesOnEdgesEpsilon, float.Epsilon);
            data.FixTrianglesOnEdgesHeightEpsilon = Mathf.Max(data.FixTrianglesOnEdgesHeightEpsilon, float.Epsilon);
          }
          ...
          if (data.ImportRegions) {
            ...
            for (int i = 0; i < data.RegionAreaIds.Count; i++) {
              ...
              if (!GUI.Toggle(rect, true, areaName)) {
                data.RegionAreaIds.Remove(areaId);
              }
              else {
                names.Remove(areaName);
              ...
              if (areaName == "Walkable" || areaName == "Not Walkable") {
                QuantumEditorGUI.Overlay(rect, "Error: Chose A Different, Non-Default Region", Color.red);
              }
            }
            ...
            if (names.Count > 0) {
              ...
              EditorGUILayout.LabelField("No Non-Default Unity Navmesh Areas Found");
            }
  ...
  public class MapNavMeshRegionEditor : UnityEditor.Editor {
    ...
    public override void OnInspectorGUI() {
      ...
      if (data.CastRegion == MapNavMeshRegion.RegionCastType.CastRegion) {
        ...
        if (data.gameObject.GetComponent<MeshRenderer>() == null && data.gameObject.GetComponent<Collider>() == null) {
          EditorGUILayout.HelpBox("MapNavMeshRegion requires a MeshRenderer or Collider (3D) to be able to cast a region onto the navmesh (depending on the geometry collection selected on the NavMeshSurface). ", MessageType.Error);
        }
        ...
        using (var change = new EditorGUI.ChangeCheckScope()) {
          ...
          if (NavMeshModifierType == null) {
            // Show upgrade tip
            EditorGUILayout.HelpBox("The Unity global navmesh feature is marked obsolete. Consider moving towards the com.unity.ai.navigation package.", MessageType.Warning);
          }
// Disable obsolete warning for 2022 or higher
#pragma warning disable CS0618 // Type or member is obsolete
#endif
  ...
  public abstract class NestedAssetBaseEditor : AssetBaseEditor {
    public override void OnInspectorGUI() {
      ...
      if (asset.Parent == null) {
        EditorGUILayout.HelpBox($"Needs to be an asset nested in a prefab.", MessageType.Error);
      }
    ...
    public override void OnInspectorGUI() {
      ...
      using (new QuantumEditorGUI.CustomEditorScope(serializedObject)) {
        using (new QuantumEditorGUI.BoxScope("Quantum Editor Settings")) {
          ...
          DrawScriptingDefineToggle(new GUIContent("Use XY as 2D Plane", "Toogles QUANTUM_XY scripting define for all platforms and will change the ProjectSettings/ProjectSettings.asset file."), "QUANTUM_XY", true);

          EditorGUI.BeginChangeCheck();
          DrawScriptingDefineToggle(new GUIContent("Remote Profiler", "Toggle QUANTUM_REMOTE_PROFILER scripting define for the current platform"), "QUANTUM_REMOTE_PROFILER");
          if (EditorGUI.EndChangeCheck()) {
...
#if !UNITY_2019_1_OR_NEWER
          {
            ...
            DrawScriptingDefineToggle(content, "QUANTUM_ADDRESSABLES", allPlatforms: true);
          }
          ...
          DrawScriptingDefineToggle(new GUIContent("Disable 2D Physics Module", "Toogles QUANTUM_DISABLE_PHYSICS2D scripting define for all platforms to make Quantum compatible with disabled Unity modules."), "QUANTUM_DISABLE_PHYSICS2D", true);
          DrawScriptingDefineToggle(new GUIContent("Disable 3D Physics Module", "Toogles QUANTUM_DISABLE_PHYSICS3D scripting define for all platforms to make Quantum compatible with disabled Unity modules."), "QUANTUM_DISABLE_PHYSICS3D", true);
          DrawScriptingDefineToggle(new GUIContent("Disable AI Module", "Toogles QUANTUM_DISABLE_AI scripting define for all platforms to make Quantum compatible with disabled Unity modules."), "QUANTUM_DISABLE_AI", true);
          DrawScriptingDefineToggle(new GUIContent("Disable Terrain Module", "Toogles QUANTUM_DISABLE_TERRAIN scripting define for all platforms to make Quantum compatible with disabled Unity modules."), "QUANTUM_DISABLE_TERRAIN", true);
#endif
        }
